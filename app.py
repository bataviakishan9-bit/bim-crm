...
# ── ADMIN: SQLITE → POSTGRES ONE-TIME IMPORT ──────────────────────────────────

@app.route("/admin/import-sqlite", methods=["GET", "POST"])
@login_required
def admin_import_sqlite():
    """One-time import of SQLite export_data.json into PostgreSQL."""
    if current_user.username != "kishan":
            ...
            try:
                ...
                for r in data.get("email_logs", []):
                    try:
                        c.execute("""
                            INSERT INTO email_logs (
                                id,lead_id,subject,body,template_used,sequence_step,
                                sent_at,status,opened,clicked,open_count,bounced,bounce_reason
                            ) VALUES (
                                %(id)s,%(lead_id)s,%(subject)s,%(body)s,%(template_used)s,
                ...
                for r in data.get("replies", []):
                    try:
                        c.execute("""
                            INSERT INTO replies (
                                id,lead_id,from_email,subject,body,priority,status,source,received_at
                            ) VALUES (
                                %(id)s,%(lead_id)s,%(from_email)s,%(subject)s,%(body)s,
                ...
                for r in data.get("tasks", []):
                    try:
                        c.execute("""
                            INSERT INTO tasks (
                                id,lead_id,subject,due_date,status,priority,description,created_at
                            ) VALUES (
                                %(id)s,%(lead_id)s,%(subject)s,%(due_date)s,
                ...
                conn.commit()
                conn.close()
                stats = {"leads": leads_ok, "email_logs": logs_ok,
                         "replies": replies_ok, "tasks": tasks_ok}
                flash(f"Import complete! Leads: {leads_ok}, Email logs: {logs_ok}, Replies: {replies_ok}, Tasks: {tasks_ok}", "success")
            except Exception as exc:
                error = str(exc)
